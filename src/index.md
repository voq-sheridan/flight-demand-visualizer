...
function buildUI() {
  ...
  sub.textContent = 'GitHub-style heatmap of hourly demand across dates. Darker red indicates higher staffing pressure.';

  // Heatmap container
  const heatmapContainer = document.createElement('div');
  heatmapContainer.className = 'heatmap-container';
  heatmapContainer.style.marginBottom = '0.65rem';
  ...
  chartCard.appendChild(heatmapContainer);
...
function buildHeatmapData(flights) {
  ...
  flights.forEach((f) => {
    ...
    slot.total += 1;
    if (f.type === 'departure') slot.departures += 1;
    if (f.type === 'arrival') slot.arrivals += 1;
    if (f.airline) slot.airlines.add(f.airline);
  });

  const dates = Array.from(dateSet).sort(); // earliest at top
  const cells = [];
  ...
  return { dates, cells, maxTotal };
}

function drawHeatmap(svg, container, heatmapData) {
  const { dates, cells, maxTotal } = heatmapData;
  ...
  const today = torontoDateKey(new Date());
  ...
  // x-axis hour labels (00..23)
  const hourLabels = d3.range(24).map((h) => String(h).padStart(2, '0'));
  g
  ...
  g
    ...
    .style('font-weight', (d) => (d === today ? '700' : '400'))
    .text((d) => {
  ...
  g
    ...
    .attr('y', (d) => margin.top + dates.indexOf(d.dateKey) * (cellSize + gap))
    .attr('width', cellSize)
    ...
    .on('mousemove', (event, d) => {
      ...
      tooltipSel
        .html(`
          ...
          <div>Departures: ${d.departures} · Arrivals: ${d.arrivals}</div>
...
function renderForSelection() {
  ...
  const heatmapData = buildHeatmapData(allFlights);
  drawHeatmap(heatmapSvg, heatmapContainer, heatmapData);
